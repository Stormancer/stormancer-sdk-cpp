...
#if defined(_MSC_VER)
#if !defined(HAVE_SET_THREAD_NAME)
...
void
ptw32_SetThreadName( DWORD dwThreadID, char* threadName)
{
...
{
  wchar_t wideName[16];
  HANDLE thread = OpenThread(THREAD_ALL_ACCESS, TRUE, dwThreadID);
  ...
  mbstowcs_s(NULL, wideName, sizeof(wideName)/sizeof(WORD), threadName, 15);
  wideName[15] = '\0';

  SetThreadName(thread, wideName);
}
#endif // HAVE_SET_THREAD_NAME
#endif
...
pthread_setname_np(pthread_t thr, const char *name, void *arg)
{
  ...
  if (Win32ThreadID)
    {
      ptw32_SetThreadName(Win32ThreadID, newname);
    }
#endif
