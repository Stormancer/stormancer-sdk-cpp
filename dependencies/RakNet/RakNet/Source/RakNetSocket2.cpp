...
RakNetSocket2* RakNetSocket2Allocator::AllocRNS2(void)
{
	...
	s2 = RakNet::OP_NEW<RNS2_360>(_FILE_AND_LINE_);
	s2->SetSocketType(RNS2T_XBOX_360);
#elif defined(_XBOX_720_WITH_XBOX_LIVE) || defined(_XBOX_720_COMPILE_AS_WINDOWS)
	s2 = RakNet::OP_NEW<RNS2_720>(_FILE_AND_LINE_);
	s2->SetSocketType(RNS2T_XBOX_720)
#elif defined(_WIN32)
...
#elif defined(_XBOX) || defined(X360)
	RNS2_360::GetMyIP(addresses);
#elif defined(_XBOX_720_WITH_XBOX_LIVE) || defined(_XBOX_720_COMPILE_AS_WINDOWS)
	RNS2_720::GetMyIP(addresses);
#elif defined(_WIN32)
...
#elif defined(_XBOX) || defined(X360)
	return RNS2_360::DomainNameToIP(domainName, ip);
#elif defined(_XBOX_720_WITH_XBOX_LIVE) || defined(_XBOX_720_COMPILE_AS_WINDOWS)
	return RNS2_720::DomainNameToIP(domainName, ip);
#elif defined(_WIN32)
	return DomainNameToIP_Berkley(domainName, ip);
#else
	return DomainNameToIP_Berkley(domainName, ip);
...
RNS2BindResult RNS2_360_720::Bind(RNS2_BerkleyBindParameters *bindParameters, const char *file, unsigned int line) { return BindShared(bindParameters, file, line); }
#endif
...
RNS2SendResult RNS2_360::Send(RNS2_SendParameters *sendParameters, const char *file, unsigned int line) {
	if (binding.protocol == IPPROTO_VDP)
		return SendWithVDP_360_720(sendParameters, 0, 0, file, line);
	else
		return Send_Windows_Linux_360NoVDP(rns2Socket, sendParameters, file, line);
}
void RNS2_360::GetMyIP(SystemAddress addresses[MAXIMUM_NUMBER_OF_INTERNAL_IDS]) { RakAssert("Unsupported" && 0); }
void RNS2_360::DomainNameToIP(const char *domainName, char ip[65]) { return DomainNameToIP_360_720(domainName, ip); }
#elif defined(_XBOX_720_WITH_XBOX_LIVE) || defined(_XBOX_720_COMPILE_AS_WINDOWS)
RNS2SendResult RNS2_720::Send(RNS2_SendParameters *sendParameters, const char *file, unsigned int line) {
...
void RNS2_720::GetMyIP(SystemAddress addresses[MAXIMUM_NUMBER_OF_INTERNAL_IDS]) { (void)addresses; RakAssert("Unsupported" && 0); }
void RNS2_720::DomainNameToIP(const char *domainName, char ip[65]) {
#if defined(_XBOX_720_WITH_XBOX_LIVE)
	return DomainNameToIP_360_720(domainName, ip);
#else
	(void)domainName;
	(void)ip;
	RakAssert("Unsupported" && 0);
#endif
}
#elif defined(_WIN32)
