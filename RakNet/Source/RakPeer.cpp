...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
StartupResult RakNet::RakPeer::Startup(unsigned int maxConnections, const DataStructures::List<SocketDescriptor>& socketDescriptorsList, int threadPriority)
{
	SocketDescriptor *socketDescriptors = &socketDescriptorsList.Get(0);
	unsigned int socketDescriptorCount = socketDescriptorsList.Size();
	...
	for (i = 0; i < socketsCreated; i++)
	{
		...
		{
			auto pollingThreadCreation = ((RNS2_Berkley*)socketList[i])->CreateRecvPollingThread(threadPriority);
#if !defined(_NO_PPLX)		
			pollingThreadCreation.then([](pplx::task<void> task)
			{
				try
				{
					task.get();
...
namespace RakNet {
	bool ProcessOfflineNetworkPacket(SystemAddress systemAddress, const char *data, const int length, RakPeer *rakPeer, RakNetSocket2* rakNetSocket, bool *isOfflineMessage, RakNet::TimeUS timeRead)
	{
		...
		if (*isOfflineMessage)
		{
				...
				|| (unsigned char)(data)[0] == ID_UNCONNECTED_PING) && length >= sizeof(unsigned char) + sizeof(RakNet::Time) + sizeof(OFFLINE_MESSAGE_DATA_ID))
			{
					...
					rakPeer->AllowIncomingConnections()) // Open connections with players
				{
					...
					for (j = 0; j < rakPeer->pluginListNTS.Size(); j++)
						rakPeer->pluginListNTS[j]->OnDirectSocketSend((const char*)outBitStream.GetData(), outBitStream.GetNumberOfBytesUsed(), systemAddress);
			...
			{
				//printf("Received ID_OPEN_CONNECTION_REPLY_1\n");
				for (i = 0; i < rakPeer->pluginListNTS.Size(); i++)
					rakPeer->pluginListNTS[i]->OnDirectSocketReceive(data, length * 8, systemAddress);
			...
			else if ((unsigned char)(data)[0] == ID_OPEN_CONNECTION_REQUEST_2)
			{
				...
				if (outcome == 1)
				{
					...
					unsigned int j;
					for (j = 0; j < rakPeer->pluginListNTS.Size(); j++)
						rakPeer->pluginListNTS[j]->OnDirectSocketSend((const char*)bsAnswer.GetData(), bsAnswer.GetNumberOfBitsUsed(), systemAddress);
