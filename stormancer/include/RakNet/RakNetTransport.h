...
#pragma region private_methods
		...
		std::shared_ptr<RakNetConnection> onConnection(RakNet::SystemAddress systemAddress, RakNet::RakNetGUID guid);
		...
		std::shared_ptr<RakNetConnection> createNewConnection(RakNet::RakNetGUID raknetGuid, std::weak_ptr<RakNet::RakPeerInterface> peer);
		std::shared_ptr<RakNetConnection> removeConnection(RakNet::RakNetGUID guid);
		void onRequestClose(std::shared_ptr<RakNetConnection> c);
...
#pragma region private_members
		...
		std::map<std::string, pplx::task_completion_event<std::weak_ptr<IConnection>>> _pendingConnections;
