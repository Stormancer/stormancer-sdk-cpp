...
namespace Stormancer
{
	...
	std::wstring stringJoin(const std::vector<std::wstring>& vector, const std::wstring& glue)
	{
	...
	{
		return stringSplit(str, std::wstring(&separator, 1));
	}
	...
	std::wstring stringTrim(const std::wstring& str2, wchar_t ch)
	{
		auto str = str2;
		std::function<int(int)> ischar = [ch](int c) { return ((c == ch) ? 1 : 0); };
		str.erase(str.begin(), find_if(str.begin(), str.end(), not1(ischar)));
