...
struct combine_latest_traits {
    ...
    static_assert(!std::is_same<decltype(checkRX<selector_type, typename ObservableN::value_type...>(0)), tag_not_valid>::value, "combine_latest Selector must be a function with the signature value_type(Observable::value_type...)");
